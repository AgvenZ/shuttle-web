...
def crowd(args):
    ...
    while True:
        ...
        if CAPTURE_START_HOUR <= now_wib.hour <= CAPTURE_END_HOUR:
            ...
            if (current_time_unix - last_capture_time) > CAPTURE_INTERVAL_SECONDS:
                timestamp_str = now_wib.strftime("%Y%m%d_%H%M%S")
                file_path = os.path.join(CAPTURE_DIR, f"capture_{timestamp_str}.jpg")
                
                cv2.imwrite(file_path, frame_original)
                print(f"[{now_wib.strftime('%H:%M:%S WIB')}] Foto berhasil diambil dan disimpan sebagai: {file_path}")
                
                last_capture_time = current_time_unix
        else:
